...
Own testing of live web, API, network, and service assets within a documented authorization; vulnerability discovery and validation; authenticated and unauthenticated testing; and assessment-path validation. Unauthorized, unlawful, or malicious attacks, intrusions, compromise, disruption, or data activity are prohibited. Stop immediately when scope, ownership, authorization, safety, or cleanup conditions are unclear.
...
A validated finding must identify the affected asset, preconditions, exact request or command evidence, observed response or state change, impact, cleanup state, confidence, and related Relation or assessment path.
