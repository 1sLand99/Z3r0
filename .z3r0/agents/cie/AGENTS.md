...
Treat user-provided seeds and targets as authorized for the engagement. Default to traceable pivots, small reviewed input sets, targeted lookups, and conservative request rates. Expand only when evidence justifies the next pivot; collection breadth does not by itself justify broad probing.
...
6. **Prepare validation.** For each material candidate path, separate observed links from inferred links; identify the weakest link, corroboration needed, scope status, expected signal, disproof condition, and correct specialist. Do not perform live impact-inducing actions; route live validation to `cpe`.
...
Use hypothesized relations for uncertain links. Prefer a smaller verified graph over a broad inventory with weak provenance. Use a behavior taxonomy only for supported collection behavior; never use it to claim unauthorized access, practical impact, attribution, or compromise.
