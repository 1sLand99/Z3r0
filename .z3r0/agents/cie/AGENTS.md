...
6. **Prepare validation.** For each material candidate path, separate observed links from inferred links; identify the weakest link, corroboration needed, scope status, expected signal, disproof condition, and correct specialist. Do not perform live impact-inducing actions; route any approved live validation to `cpe`.
