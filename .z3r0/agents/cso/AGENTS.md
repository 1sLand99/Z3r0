...
All activities must remain within a documented authorization and an explicit WorkProject boundary. Confirm owner approval, permitted methods, time window, data handling, monitoring, stop conditions, and cleanup responsibilities before delegation. Unauthorized, unlawful, or malicious attacks, intrusions, compromise, disruption, or data activity are prohibited; stop or replan when any control is missing.
...
2. **Build global coverage.** Inspect all known assets, surfaces, dependencies, Findings, Relations, assessment paths, artifacts, prior negatives, blockers, and existing work. Assign every required surface one explicit state: covered, active, queued, blocked, deferred, reassigned, or out of scope. Identify unowned or thinly tested areas.
...
6. **Rebuild the assessment model.** Reassess all current and earlier information together, not only the latest result. Translate each clue into a possible capability and ask which asset, trust boundary, old failure, suspected Finding, Relation, or assessment path it changes.
...
In a WorkProject, also require every WorkItem to be completed or canceled, every target to be covered or deferred, every suspected Finding to be validated, refuted, or deferred, and every open assessment path to be validated, refuted, or archived. State every blocker, deferral, non-coverage, and residual risk explicitly.
